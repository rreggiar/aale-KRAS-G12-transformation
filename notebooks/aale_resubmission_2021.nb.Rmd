...
salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < -0.5, gene %in% meta.data$trono.krab.znfs$gene) %>%  #, gene %in% uniq_ctrl_peak_genes$`Gene Name`) %>% 
  # filter(gene %in% filter(salmon.quant$hbec$lacz_v_v12$deseq$de, padj < 0.05, log2FoldChange > 0)$gene) %>% 
  select(gene_id) %>% unlist() %>% unname() -> de_list

salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < -0.5, grepl('^HOX', gene)) %>% 
  # filter(gene %in% filter(salmon.quant$hbec$lacz_v_v12$deseq$de, padj < 0.05, log2FoldChange > 0)$gene) %>% 
  select(gene_id) %>% unlist() %>% unname() -> de_list

salmon.quant$aale$intra$deseq$de %>% 
  # filter(gene == 'ZNF563') %>% 
  filter(padj > 0.05, gene %in% meta.data$trono.krab.znfs$gene) %>%
...
znf_promoter_seqs <- 
  GenomicRanges::promoters(znf_de_gr, upstream = 200, downstream = 200) %>% 
  memes::get_sequence(genome = BSgenome.Hsapiens.UCSC.hg38)

znf_promoter_seqs_hbec <- 
  GenomicRanges::promoters(znf_de_gr_hbec, upstream = 200, downstream = 200) %>% 
  memes::get_sequence(genome = BSgenome.Hsapiens.UCSC.hg38)

znf_ame_out <- memes::runAme(znf_promoter_seqs, control = znf_promoter_seqs_hbec)
...
meme_res <- memes::runDreme(input = promoter_seqs, control = 'shuffle', e = 50)
meme_res %>% 
  memes::runTomTom()
...
consensus_atac_boolean_anno %>% 
  ...
  mutate(strand = '+') %>%
  select(chr, start, end, strand, name = `Gene Name`) %>%  #-> ctrl_znf_uniq_peak
