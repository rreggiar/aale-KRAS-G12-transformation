...
working_toil_counts %>% 
  ...
  select(gene) %>% 
  distinct() %>% 
  mutate(ISRE = ifelse(gene %in% isre_fimo_results$gene, T, F)) %>% 
  column_to_rownames('gene') %>% 
...
ComplexHeatmap::Heatmap(t(toil_hm_counts), 
                        ...
                        height = unit((panel.figure.height/2)/1.247408, 'mm'), 
                        width = unit((panel.figure.width)/1.376471, 'mm')) -> toil_hm_plt
