...
consensus_atac_boolean_anno %>% 
  ...
  # group_by(annotation) %>%
  group_by(`Gene Name`) %>%
  tally() -> uniq_peak_genes#%>% filter(grepl('ZNF', `Gene Name`)) %>% 
  # arrange(-n)
...
memes::runAme(input = ifn_uniq_seqs, control = other_uniq_seqs) %>% 
  memes::plot_ame_heatmap()
  
  
salmon.quant$aale$intra$deseq$de %>% filter(gene %in% uniq_peak_genes$`Gene Name`) %>% View()
