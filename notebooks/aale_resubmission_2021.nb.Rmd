...
```{r building quant data, eval=eval_build_datasets}
...
```
### CALL: run.de.seq
```{r running deseq, eval=eval_build_datasets}
if (build_salmon.quant == T) { 
...
```
### CALL: a549 de seq
```{r a549 de seq, cache=TRUE, results=FALSE}
...
```
### CALL: plot_pca
```{r running and plotting pca, eval=eval_build_datasets}
if (build_salmon.quant == T) { 
...
```{r extract useful data, results=FALSE}
irf9_fimo_results %>%
...
```
### complex plotting
```{r Figure 1 heatmap, fig.show='hide'}
...
### complex plotting
```{r Figure 5 heatmap, results=FALSE}
plot_complex_heatmap <- function(filter_values, show_genes = F, split_vector = NULL, split_title = NULL) {
...
## Figure S1. Mutant KRAS G12D induces global transcriptional transformation; Figure 1. 
```{r Figure S1}
salmon.quant$aale$intra$deseq$de %>% 
  mutate(padj = ifelse(padj == 0, 
                       min(filter(salmon.quant$aale$intra$deseq$de, padj != 0)$padj), padj)) %>% 
...
## Figure S2. Mutant KRAS G12D causes significant shifts in extracellular vesicle RNA content; Figure 3.
```{r Figure S2}
...
## Figure S3. TE RNAs activated by mutant KRAS are enriched for KZNF motifs; Figures 3 & 4. 
```{r Figure S3}
...
## Figure S4. KZNF binding scores highlight potential regulation of differentially expressed TEs in AALE EVs and HBEC cell line; Figures 3 & 4.
```{r Figure S4}
fig_S5_patch <- "
AA
BB
...
## Figure S5. Downregulated KZNFs target upregulated TEs; Figure 1, 3 & 4.
```{r Figure S5}
...
## Figure S6. Mutant KRAS-regulated KZNFs repress ISGs and TE RNAs; Figures 1, 2, 3 & 4.
```{r Figure S6}
a549_de$de_682 %>% 
