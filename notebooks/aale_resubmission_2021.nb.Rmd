...
salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < 0, gene %in% c(jak_stat)) %>% 
...
salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < -0.5, gene %in% meta.data$trono.krab.znfs$gene) %>% 
  # filter(gene %in% filter(salmon.quant$hbec$lacz_v_v12$deseq$de, padj < 0.05, log2FoldChange > 0)$gene) %>% 
  select(gene_id) %>% unlist() %>% unname() -> de_list
...
a549_te_de$de_682 %>% 
  ...
  geom_point(alpha = 0.6) + 
  scale_color_viridis_d(direction = -1) +
  ggrepel::geom_text_repel(aes(label = ifelse(log2FoldChange < -0.5, gene_id, '')), show.legend = F) +
  facet_col(~oe)
...
bamsignals::bamCoverage(kras_bam_path, te_ranges_tmp, paired.end = 'extend') -> kras_de_te_profile
bamsignals::bamCoverage(ctrl_bam_path, te_ranges_tmp, paired.end = 'extend') -> ctrl_de_te_profile
...
znf_score %>% 
  ...
  filter(TE %in% filter(upreg_te, context %in% c('aale_intra', 'aale_exo', 'h358_intra'))$gene,
         gene %in% filter(salmon.quant$aale$intra$deseq$de, padj < 0.05, 
