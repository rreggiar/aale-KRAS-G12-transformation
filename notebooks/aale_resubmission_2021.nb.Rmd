...
ucsc.rmsk.salmon.quant$aale$intra$deseq$de %>%
  ...
  merge(ucsc.rmsk.salmon.quant$hbec$lacz_v_v12$deseq$de %>%
    filter(!gene %in% meta.data$gencode.35.tx.to.gene$gene,
           padj < 0.05) %>%
...
imap(list('aale_intra' = ucsc.rmsk.salmon.quant$aale$intra$deseq$de,
          'aale_exo' = ucsc.rmsk.salmon.quant$aale$exo$deseq$de,
          'h358_intra' = ucsc.rmsk.salmon.quant$hbec$lacz_v_v12$deseq$de), function(x, x_name) {
           x %>% 
             filter(!gene %in% meta.data$gencode.35.tx.to.gene$gene,
...
te_ranges_list %>% 
  lapply(function(te_ranges) {
    ...
    if(length(te_ranges) > 0 & length(te_ranges) < 4000) {
      ...
      memes::runAme(te_seqs, control = dn_te_seqs) -> te_motif_enrich
...
znf_score %>% 
  ...
  filter(TE %in% filter(upreg_te, context %in% c('aale_intra', 'aale_exo', 'h358_intra'))$gene,
         gene %in% filter(salmon.quant$aale$intra$deseq$de, padj < 0.05, log2FoldChange < -0.75)$gene) -> znf_score
...
znf_score %>% 
  group_by(gene) %>% 
  filter(TE %in% filter(upreg_te, grepl('aale_intra', context))$gene) %>% 
  top_n(1, wt = score) %>% 
  merge(salmon.quant$aale$intra$deseq$de, by = 'gene') %>% 
  filter(padj < 0.05) %>% 
