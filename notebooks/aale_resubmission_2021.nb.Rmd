...
salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < -4.5, gene %in% meta.data$trono.krab.znfs$gene) %>%  #, gene %in% uniq_ctrl_peak_genes$`Gene Name`) %>% 
  # filter(gene %in% filter(salmon.quant$hbec$lacz_v_v12$deseq$de, padj < 0.05, log2FoldChange > 0)$gene) %>% 
  select(gene_id) %>% unlist() %>% unname() -> de_list
...
promoter_seqs_hbec <- 
  GenomicRanges::promoters(znf_de_gr_hbec, upstream = 300, downstream = 300) %>% 
  memes::get_sequence(genome = BSgenome.Hsapiens.UCSC.hg38)
...
job <- rGREAT::submitGreatJob(uniq_ctrl_peak_input_bed, species = 'hg38', bg = bg_peak_input_bed)

tb <- rGREAT::getEnrichmentTables(job, download_by = 'tsv')
  
base::do.call(rbind, tb) %>% 
  group_by(Ontology) %>% View()
  top_n(10, -BinomFdrQ) %>% 
  filter(HyperFdrQ < 0.05) %>% View()
  ggplot(aes(ObsGenes, -log10(BinomFdrQ), color = Ontology, label = Desc)) + 
  geom_point() + 
  scale_color_viridis_d() +
