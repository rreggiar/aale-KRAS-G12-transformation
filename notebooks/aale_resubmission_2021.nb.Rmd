...
# values(gr) <- cbind(values(gr), Biostrings::letterFrequency(x = seqs, letters = "GC", as.prob = TRUE))
# 
# sub_gr <- 
#   gr[values(gr)[, 'gene_id'] %in% filter(salmon.quant$aale$intra$deseq$de, padj < 0.01, log2FoldChange > 1)$gene_id]

# seqs <- getSeq(BSgenome.Hsapiens.UCSC.hg38, ifn_de_gr)

promoter_seqs <- 
  GenomicFeatures::extractUpstreamSeqs(x = BSgenome.Hsapiens.UCSC.hg38, genes = ifn_de_gr, width = 1000)

promoter_seqs_hbec <- 
  GenomicFeatures::extractUpstreamSeqs(x = BSgenome.Hsapiens.UCSC.hg38, genes = ifn_de_gr_hbec, width = 1000)
...
irf1_motif <- MotifDb::query(MotifDb::MotifDb, "IRF1") %>% 
  universalmotif::convert_motifs()

irf1_motif <- irf1_motif[[3]]

fos_motif <- MotifDb::query(MotifDb::MotifDb, "AP-1") %>% 
  universalmotif::convert_motifs()

stat2_motif <- MotifDb::query(MotifDb::MotifDb, 'STAT2_HUMAN.H11MO.0.A') %>% 
  universalmotif::convert_motifs()
...
stat2_fimo_results <- memes::runFimo(promoter_seqs, stat2_motif, thresh = 1e-3) %>% 
  as.data.frame() %>% 
  merge(salmon.quant$aale$intra$deseq$de, by.x = 'seqnames', by.y = 'gene_id') 
  # select(gene) %>% distinct()
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  stat_compare_means(label.x.npc = 0.55, label.y.npc = 0.06) + 
  xlab('IRF9 motif')
...
working_toil_counts %>% 
  ...
  ComplexHeatmap::rowAnnotation(df = . , 
                                ...
                                simple_anno_size = unit(1.5, "mm")) -> toil_sample_anno
...
working_toil_counts %>% 
  ...
  ComplexHeatmap::columnAnnotation(df =.,
                                ...
                                simple_anno_size = unit(1.5, "mm")) -> gene_motif_anno
...
ComplexHeatmap::Heatmap(t(toil_hm_counts), 
                        ...
                        row_dend_width = unit(4, 'mm')) -> toil_hm_plt
                        # height = unit((panel.figure.height/2)/1.33, 'mm'), 
                        # width = unit((panel.figure.width)/1.619, 'mm')) #-> toil_hm_plt

size = calc_ht_size(toil_hm_plt)
size

pdf(file = file.path(figure.dir, 'fig.1', 'toil_hm.pdf'),
    width=unit(panel.figure.width, 'mm'), height=unit(panel.figure.height/2, 'mm'), family = 'Helvetica')
...
tx.ome.cov.plt <- function(tx.counts){
  tx.counts %>% 
    ...
    dplyr::select(biotype, names, condition, biotype.count) %>% 
    distinct() %>% 
...
extract.leading.edge <- function(fgsea, deseq){
  ...
  deseq %>% 
    ...
    dplyr::select(gene) %>% unlist() %>% unname() -> g12c.tmp
...
kras_atac_anno_peaks <- 
  read_tsv(file.path(output.data.dir, 
                     'atac_out/bwa/mergedLibrary/macs/broadPeak/kras_R1.mLb.clN_peaks.annotatePeaks.txt'))

consensus_atac_anno_peak_counts <- 
  read_tsv(file.path(output.data.dir, 
                     'atac_out/bwa/mergedLibrary/macs/broadPeak/consensus/consensus_peaks.mLb.clN.featureCounts.txt'),
           skip = 1)

consensus_atac_boolean_anno <- 
  read_tsv(file.path(output.data.dir, 
                     'atac_out/bwa/mergedLibrary/macs/broadPeak/consensus/consensus_peaks.mLb.clN.boolean.annotatePeaks.txt'))
oreagnno_tfbs <- 
  read_tsv(file.path(output.data.dir, 
                     'oreganno_tfbs_int_consensus_anno_peaks_wb_f0.7.tsv'), 
           col_names = c('chr', 'tfbs_start', 'tfbs_end', 'tf', 'junk_1', 'tfbs_strand', 
                         'junk_2', 'peak_start', 'peak_end', 'interval_id'))

rmsk_int_cons <- 
  read_tsv(file.path(output.data.dir, 
                     'rmsk_int_consensus_anno_peaks_wb_f0.7_r.tsv'), 
           col_names = c('chr', 'tfbs_start', 'tfbs_end', 'tf', 'junk_1', 'tfbs_strand', 
                         'junk_2', 'peak_start', 'peak_end', 'interval_id'))

oreagnno_tfbs %>% 
  dplyr::select(-junk_1, -junk_2) %>% 
  merge(consensus_atac_boolean_anno %>% 
  filter(`Gene Name` %in% filter(salmon.quant$aale$deseq$de, abs(log2FoldChange) > 0.25)$gene), 
  by = 'interval_id') -> tfbs_boolean_peaks
