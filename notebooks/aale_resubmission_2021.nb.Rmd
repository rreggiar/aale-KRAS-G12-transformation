...
rbind(kras_de_prom_df, ctrl_de_prom_df) %>%
  ggplot(aes(position, cov, group = condition, color = condition)) +
    stat_summary(geom="ribbon", fun.data=mean_cl_boot, width=0.1, conf.int=0.95, fill='grey', alpha = 0.4)+
    # stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, alpha = 0.5, size = rel(0.1)) +
...
consensus_atac_boolean_anno %>% 
  ...
  filter(`Gene Name` %in% aale_ifn$gene) %>%
  separate(Annotation, sep = '\\(', into = c('annotation', NA)) %>% 
  # filter(annotation == "promoter-TSS ") %>%
  group_by(annotation) %>%
  tally() #%>% filter(grepl('ZNF', `Gene Name`)) %>% 
  arrange(-n)

HelloRanges::R_bedtools_intersect(kras_atac_bed, irf9_bed, wa = T, F = 0.25, s = T)

{
    genome <- Seqinfo(genome = NA_character_)
    gr_a <- kras_atac_bed
    gr_b <- irf9_bed
    pairs <- findOverlapPairs(gr_a, gr_b, ignore.strand = FALSE)
    olap <- pintersect(pairs, ignore.strand = FALSE)
    keep <- width(olap)/width(second(pairs)) >= 0
    pairs <- pairs[keep]
    ans <- first(pairs)
    ans
}
...
consensus_atac_boolean_anno %>% 
  filter(interval_id %in% as.data.frame(values(ans))$name) %>% 
  View()

  
merge(consensus_atac_anno_peak_counts, by.x = c('chr', 'start.x', 'end.x'), by.y = c('Chr', 'Start', 'End'))
