...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_text_repel(aes(label = ifelse(`unique peaks` == 'ISG' | log2FoldChange > 6, gene, '')), 
                  size = txt.mm_to_pts(0.8), show.legend = F) + 
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') + 
  theme(legend.position = c(0.99, 0.99)) -> plt_2B.plt
...
base::do.call(rbind, tb) %>% 
  ...
  ggplot(aes(NumFgGenesHit, -log10(HyperFdrQ), label = Desc)) + 
  geom_point() + 
  ggrepel::geom_text_repel(show.legend = F, color = 'black', size = txt.mm_to_pts(0.6)) +
  xlab('observed genes') +
  facet_row(~Ontology, scales = 'free') -> plt_2D.plt
