...
options(meme_db = system.file(file.path(output.data.dir, 'referenc.data', 
...
isre_fimo_results <- memes::runFimo(promoter_seqs, isre_motif, thresh = 1e-4) %>% 
  as.data.frame() %>% 
  merge(salmon.quant$aale$intra$deseq$de, by.x = 'seqnames', by.y = 'gene_id') %>%
  select(gene) %>% distinct()

fos_fimo_results <- memes::runFimo(promoter_seqs, fos_motif, thresh = 1e-4) %>% 
  as.data.frame() %>% 
  merge(salmon.quant$aale$intra$deseq$de, by.x = 'seqnames', by.y = 'gene_id') %>%
  select(gene) %>% distinct()
...
ht_exp <- as_huxtable(x = exp_tbl %>% select(pathway, padj, NES, coverage))
...
exp_tbl %>% 
  ...
  fontsize(size = 10) %>% 
  width(width = panel.figure.width, unit = 'mm') %>% 
  height(height = panel.figure.height/3, unit = 'mm') %>% 
  flextable::width(j = c('padj'), width = 0.6) %>% 
  # flextable
  save_as_image(path = file.path(figure.dir, 'fig.1', 'gsea_table.png'), webshot = 'webshot')
...
ht_exp %>% 
  set_all_padding(4) %>% 
      ...
      set_width(0.4) %>% huxtable::print_screen()

kableExtra::kbl(exp_tbl %>% select(pathway, padj, NES, coverage)) %>% 
  kableExtra::kable_paper(html_font = 'helvetica')
