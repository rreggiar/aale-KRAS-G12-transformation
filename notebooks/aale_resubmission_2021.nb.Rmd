...
plot_big_wig_coverage <- function(chr, start, stop, label_diff = 25, gene_name, break_denom) { 
  ...
  first_exon_label_pos <- first_exon_start - label_diff
  
  first_gene_name <- attr(tx_tbl, 'gene')
  
  start_stop_df <- data.frame(x = first_exon_start,
  ...
  track_data$KRAS %>% 
    ...
    scale_x_continuous(breaks = seq(min(as.data.frame(track_data$KRAS$start)), 
                              max(as.data.frame(track_data$KRAS$end)), break_step), labels = comma) +
  ...
  ggplot(data = start_stop_df, 
    ...
    # geom_vline(xintercept = seq(region_start, region_stop, break_step/2), alpha = 1, color = 'grey') +
    geom_segment(aes(xend = gene_line_end), 
                 color = 'darkblue', size = 0.75, alpha = 0.1, arrow = arrow()) +
