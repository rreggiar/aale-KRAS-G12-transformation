...
ifn_de_gr <- gr[names(gr) %in% bg_de_list]
...
promoter_seqs_hbec <- 
  GenomicFeatures::getPromoterSeq(subject = BSgenome.Hsapiens.UCSC.hg38, 
                                   query = ng_de_gr, upstream = 1000, downstream = 500)
...
salmon.quant$aale$intra$deseq$de %>% 
  filter(padj < 0.05, log2FoldChange < -1.5, gene %in% meta.data$trono.krab.znfs$gene) %>%  #, gene %in% uniq_ctrl_peak_genes$`Gene Name`) %>% 
  # filter(gene %in% filter(salmon.quant$hbec$lacz_v_v12$deseq$de, padj < 0.05, log2FoldChange > 0)$gene) %>% 
  select(gene_id) %>% unlist() %>% unname() -> de_list
...
salmon.quant$aale$intra$deseq$de %>% 
  # filter(gene == 'ZNF563') %>% 
  filter(padj > 0.1) %>%  #, log2FoldChange < -1.5, !gene %in% meta.data$trono.krab.znfs$gene) %>%
...
promoter_seqs <- 
  GenomicFeatures::getPromoterSeq(subject = BSgenome.Hsapiens.UCSC.hg38, 
                                  query = znf_de_gr, upstream = 1000, downstream = 500)

promoter_seqs_hbec <- 
  GenomicFeatures::getPromoterSeq(subject = BSgenome.Hsapiens.UCSC.hg38, 
                                   query = znf_de_gr_hbec, upstream = 1000, downstream = 500)
...
tomtom_res <- create_motif('TACAAAAW', altname = 'test', type = 'PCM') %>% memes::runTomTom()
