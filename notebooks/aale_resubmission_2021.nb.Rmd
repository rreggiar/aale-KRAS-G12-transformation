...
###
```{r}
aale_sc <- subset(aale_sc, subset = nFeature_RNA > 2100 & percent.mt < 7.5)
VlnPlot(aale_sc, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
...
aale_sc <- RunUMAP(aale_sc, dims = 1:dims, n.neighbors = 10)
...
aale.markers %>% 
  ...
  group_by(cluster) %>% 
  do(broom::glance(lm(data=., formula = log2FoldChange ~ avg_log2FC)))



  nest(avg_log2FC, log2FoldChange) %>% 
  mutate(model = map(data, ~lm(avg_log2FC ~ log2FoldChange, data = .))) %>% 
  mutate(tidy_model = map(model, broom::tidy)) %>% 
  select(-data, -model) %>% unnest() %>% select(cluster, estimate, statistic, p.value) %>% unique()

lm(avg_log2FC ~ log2FoldChange, clust.8.de %>% merge(salmon.quant$aale$deseq$de, by = 'gene'))
...
FindMarkers(aale_sc, ident.1 = 0, ident.2 = c(5,4), min.pct = 0.25, logfc.threshold = 0.5, only.pos = T) %>% head()
```
...
msig.df$HALLMARK_INTERFERON_GAMMA_RESPONSE %>% 
