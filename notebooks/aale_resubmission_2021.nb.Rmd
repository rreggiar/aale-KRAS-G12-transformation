...
patch_fig_height = unit(279.4 , 'mm')
...
lapply(c(1,2), function(i) { 
  ...
  geom_text(stat='count', aes(label=after_stat(count)), size = base_size*0.6*txt.mm_to_pts)
...
salmon.quant$aale$intra$deseq$H.fgsea %>% 
  ...
  ggplot(aes(NES, rank, color = padj)) + 
  geom_point(size = 4) +
  ...
  ylab('gsea rank') + 
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') +
  theme(legend.position = c(0.95, 0.16)) -> plt_1B.plt
...
wrap_plots(plt_1A.plt,
           ...
           patchwork::wrap_elements(full = 
                                      grid::grid.grabExpr(ComplexHeatmap::draw(plt_1E.plt,
                                                                               heatmap_legend_side = 'right'), 
                                                                               width = patch_fig_width)), 
           design = fig_1_patch) + 
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = rel(0.75)),
        plot.tag.position = c(0.01, 0.99)) -> figure_1_patchwork.fig


ggsave(plot = figure_1_patchwork.fig,
       filename = file.path(figure.dir, 'fig.1', 'figure_1_pathwork.pdf'),
...
working_toil_counts %>% 
  ...
  # filter(gene %in% aale_hbec_de$gene) %>%
  group_by(gene) %>%
...
ComplexHeatmap::rowAnnotation(
  ...
  annotation_name_gp = grid::gpar(fontsize = 7)) -> log2fc_ha
...
working_toil_counts %>% 
  ...
  ComplexHeatmap::rowAnnotation(df =.,
                                ...
                                show_legend = F,
                                annotation_name_gp = grid::gpar(fontsize = 7),
                                simple_anno_size = unit(1.75, "mm")) -> gene_motif_anno
...
ComplexHeatmap::Heatmap(toil_hm_counts, 
                        ...
                        heatmap_width = patch_fig_width*0.875) -> plt_1E.plt
...
salmon.quant$aale$intra$deseq$H.fgsea %>% 
  ...
  arrange(-NES, padj) %>% 
  filter(!grepl('ZNF', pathway)) %>% 
  ggplot(aes(NES, reorder(pathway, NES), color = padj)) + 
  geom_point(size = 4) +
  ...
  ylab('') + 
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') +
  theme(legend.position = c(0.95, 0.16)) -> plt_1A.plt
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  mutate(gene_set = ifelse(gene %in% ifn_gene_list,'IFN', 'other'),
         gene_set = ifelse(gene %in% msig.df$HALLMARK_KRAS_SIGNALING_UP | gene == 'KRAS', 'KRAS', gene_set),
         gene_set = ifelse(gene %in% meta.data$trono.krab.znfs$gene, 'ZNF', gene_set)) %>% 
  mutate(gene_set = factor(gene_set, levels = c('IFN','ZNF','KRAS', 'other'))) %>% 
  filter(padj < 0.05, gene_set != 'other') %>% 
  mutate(kras = ifelse(gene == 'KRAS', T, F)) %>% 
  ggplot(aes(log2FoldChange, -log10(padj), label = gene, color = kras)) + 
  geom_point(alpha = 0.6) + 
  ...
  facet_col(~gene_set, scales = 'free_y') +
  scale_color_manual(values = c('black', 'red'), guide = 'none') +
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') -> plt_1B.plt
...
# panel.save(figure = 1, name = 'de_scatter_ifn', width = 'single', height = 'double')
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  mutate(ISG = ifelse(gene %in% c(ifn_a, ifn_g), 'ISG', 'other')) %>% 
  ggplot(aes(log2FoldChange, -log10(padj), label = gene, color = ISG)) + 
  geom_point() +
  scale_color_manual(values = c('black', 'grey')) +
  geom_text_repel(color = 'black', aes(label = ifelse(ISG == 'ISG' | log2FoldChange > 6, gene, ''))) + 
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') + 
  theme(legend.position = c(0.99, 0.99), legend.title = element_blank()) 

panel.save(figure = 2, 
           name = 'unqiq_kras_peaks_volcano_isg', width = 'single', height = 'single')
...
plot_big_wig_coverage <- function(chr, start, stop, 
                                  finish_plot = F, gene_name) { 
  ...
  ggplot(data = start_stop_df, 
    ...
    theme(axis.line.y = element_blank(), 
          ...
          axis.text.x = element_blank(),
          axis.ticks.x = element_blank()) -> B.plt
  ...
  track_data$CTRL %>% 
    ...
    scale_y_continuous(limits = c(0, max(max_vector)), 
                      expand = c(0,0), breaks = get_breaks(n = 4)) +
    scale_x_continuous(labels = comma, breaks = get_breaks(n = 2)) +
    ylab('coverage') + xlab(chr) +
    geom_area(fill = alpha(viridis(3)[1], 1), color = alpha(viridis(3)[1], 1)) +
    theme(axis.line.x = element_blank()) -> C.plt
...
plot_big_wig_coverage(chr = 'chr14', start = 24163100, stop = 24164100, 
                      gene_name = 'IRF9')

plot_big_wig_coverage(chr = 'chr14', start = 94107978, stop = 94113941, 
                      label_diff = 170, gene_name = 'IFI27', break_denom = 4)

plot_big_wig_coverage(chr = 'chr12', start = 112971378, stop = 112985846, 
                      label_diff = 170, gene_name = 'OAS2', break_denom = 4)

plot_big_wig_coverage(chr = 'chr2', start = 6865371, stop = 6878717,
                      gene_name = 'RSAD2')

plot_big_wig_coverage(chr = 'chr21', start = 41418699, stop = 41429198,
                      gene_name = 'MX1')

plot_big_wig_coverage(chr = 'chr1', start = 78648360, stop = 78651764,
                      gene_name = 'IFI44')

plot_big_wig_coverage(chr = 'chr12', start = 112903592, stop = 112909610,
                      gene_name = 'OAS1')

plot_big_wig_coverage(chr = 'chr10', start = 89388968, stop = 89397133,
                      gene_name = 'IFIT1')

znf90_atac.plt <- plot_big_wig_coverage(chr = 'chr19', start = 20070264, stop = 20088302,
                                        gene_name = 'ZNF90')

znf736_atac.plt <- plot_big_wig_coverage(chr = 'chr7', start = 64297134, stop = 64316493,
                                         gene_name = 'ZNF736')

znf682_atac.plt <- plot_big_wig_coverage(chr = 'chr19', start = 20037000, stop = 20054000,
                                         gene_name = 'ZNF682')

znf826P_atac.plt <- plot_big_wig_coverage(chr = 'chr19', start = 20418153, stop = 20430548,
                                         gene_name = 'ZNF826P')

# MGMT_atac.plt <- plot_big_wig_coverage(chr = 'chr10', start = 129459369, stop = 129473612,
#                                          gene_name = 'MGMT')
...
collected_y.plt <- 
  ggplot(data.frame(l = 'coverage', x = 1, y = 1)) +
      geom_text(aes(x, y, label = l), angle = 90) + 
      theme_void() +
      coord_cartesian(clip = "off")
  
wrap_plots(znf736_atac.plt,
           znf682_atac.plt,
           znf826P_atac.plt,
           znf90_atac.plt,
           collected_y.plt,
...
panel.save(figure = 2, 
           name = 'test_patch_znf_ataac', width = 'double', height = 'double')
...
wrap_plots(znf736_atac.plt,
           ...
           design = znf_atac_layout)
panel.save(figure = 2, 
           name = 'test_patch_znf_ataac', width = 'double', height = 'single')
