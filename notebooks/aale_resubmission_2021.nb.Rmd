...
aale_ifn_chm_no_rownames <- plot_complex_heatmap(unique_peak_aale_ifn_list, 
                                                 show_genes = F, split_vector = 2, split_title = "cluster_%s")
...
salmon.quant$aale$exo$deseq$H.fgsea %>% 
  ...
  mutate(pathway = str_replace_all(pathway, 'HALLMARK ', ''),
         ...
         NES = round(NES, 3)) %>% 
  filter(!grepl('ZNF', pathway)) %>% 
  ggplot(aes(NES, context, color = padj)) + 
  geom_point(size = 4) +
