...
consensus_atac_boolean_anno %>% 
  ...
  # filter(`Gene Name` %in% filter(salmon.quant$aale$intra$deseq$de, padj < 0.05, log2FoldChange <= -1)$gene) %>%
  # filter(`Gene Name` %in% c(ifn_g, ifn_a)) %>%
  separate(Annotation, sep = '\\(', into = c('annotation', NA)) %>% 
  filter(annotation == "promoter-TSS ") %>%
  # group_by(annotation) %>%
  group_by(`Gene Name`) %>%
  tally() -> uniq_ctrl_peak_genes#%>% filter(grepl('ZNF', `Gene Name`)) %>% 
  # arrange(-n)
...
plot_big_wig_coverage(loci = 'chr14:24,158,954-24,163,321', gene_name = 'IRF9' , y_lab = 'norm.\nread depth', lab_tag = 'C') -> irf9_atac.plt
...
plot_big_wig_coverage(loci = 'chr12:112,978,011-112,986,730', gene_name = 'OAS2' , y_lab = 'norm.\nread depth') -> oas2_atac.plt
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_text_repel(aes(label = ifelse(`unique peaks` == 'ISG' | log2FoldChange > 6, gene, '')), 
                  size = txt.mm_to_pts(0.8), show.legend = F) + 
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') + 
  theme(legend.position = c(0.3, 0.99)) -> plt_2B.plt
...
base::do.call(rbind, tb) %>% 
  ...
  filter(HyperFdrQ < 0.05,
         ...
         !grepl('part', Desc)) %>% 
  ggplot(aes(-log10(HyperFdrQ), FgRegionsHit, label = Desc)) + 
  geom_point() +
  geom_text_repel(size = txt.mm_to_pts(0.8), show.legend = F) +
  ggforce::facet_row(~Ontology) +
  ylab('observed regions') -> plt_2D.plt
...
#          grepl('interferon|virus|dsRNA|oligo|^ex', Desc),
#          !grepl('part', Desc)) %>% 
#   # mutate(Desc = str_replace_all(Desc, ' ', '\n')) %>% 
#   ggplot(aes(-log10(HyperFdrQ), reorder(Desc, -HyperFdrQ), label = Desc)) + 
#   geom_point(size = 4) +
#   geom_segment(aes(x=0, xend = -log10(HyperFdrQ), y=Desc, yend=Desc)) +
...
#   # ylab('') +
#   ylab('GO Cellular Component') -> plt_2D.plt
# 
# base::do.call(rbind, tb) %>% 
#   group_by(Ontology) %>% 
#   top_n(30, -HyperFdrQ) %>%
...
#         plot.title = element_text(size = rel(0.7), face = 'plain')) +
#   # ylab('') +
#   ylab('GO Molecular Function') -> plt_2E.plt
# 
# base::do.call(rbind, tb) %>% 
#   group_by(Ontology) %>% 
#   top_n(30, -HyperFdrQ) %>%
...
#          grepl('interferon|virus', Desc)) %>% 
#   mutate(Desc = str_replace_all(Desc, 'pathway', '')) %>%
#   ggplot(aes(-log10(HyperFdrQ), reorder(Desc, -HyperFdrQ), label = Desc)) + 
#   geom_point(size = 4) +
#   geom_segment(aes(x=0, xend = -log10(HyperFdrQ), y=Desc, yend=Desc)) +
#   geom_text(aes(label = Desc, 
...
#         plot.title = element_text(size = rel(0.7), face = 'plain')) +
#   # ylab('') +
#   ylab('GO Biological Process') -> plt_S2F.plt
...
wrap_plots(plt_2A.plt + labs(tag = 'A'),
           plt_2B.plt + labs(tag = 'B'),
           ifn_atac_6.plt,
           plt_2D.plt + labs(tag = 'D'),
           # plt_2E.plt + labs(tag = 'E'),
           # plt_S2F.plt + labs(tag = 'F'),
           heights = c(1,1.5,0.50),
           design = fig_2_patch) -> figure_2_patchwork.fig
...
base::do.call(rbind, tb) %>% 
  ...
  geom_point(size = 4) +
  geom_segment(aes(x=0, xend = -log10(HyperFdrQ), y=Desc, yend=Desc)) +
  geom_text(aes(label = NumFgGenesHit, 
              x = -log10(HyperFdrQ) - 0.75), 
          nudge_y = 0.15, 
          color = 'black', 
          size = txt.mm_to_pts(0.8)) +
  ylab('') +
  ggtitle('GO Biological Process') -> plt_S2A.plt

salmon.quant$aale$intra$deseq$counts %>% 
  filter(grepl('^IFN', gene)) %>% 
  select(gene, contains('.')) %>% 
  gather(names, counts, -gene) %>% 
  merge(salmon.quant$aale$intra$deseq$col.data, by = 'names') %>% 
  ggplot(aes(reorder(gene, counts), counts, color = condition)) + 
  scale_color_manual(values = c(viridis(3)[1], viridis(3)[2])) +
...
plot_big_wig_coverage(loci = 'chr19:39,294,877-39,297,773', 
                      gene_name = 'IFNL1', y_lab = 'norm.\nread depth', lab_tag = 'C') -> ifnl1_atac.plt
...
wrap_plots(plt_S2A.plt,
           plt_S2B.plt,
           ifnl1_atac.plt,
           patchwork::plot_spacer(),
           design = fig_S2_patch) -> figure_S2_patchwork.fig
...
salmon.quant$aale$exo$deseq$de %>% 
  ...
  ylab('intracellular log2FC') + xlab('extracellular log2FC') -> plt_3B.plt
...
salmon.quant$aale$exo$deseq$de %>% 
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(abs(log2FoldChange) > 1, gene, '')), size = txt.mm_to_pts(0.8),
                           show.legend = F) +
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 'dashed') + 
  theme(legend.position = c(0.2,0.99)) -> plt_3C.plt
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_bar() + 
  ggupset::scale_x_upset() +
  geom_text(stat='count', aes(label=after_stat(count)), vjust = -0.5, size = txt.mm_to_pts(0.8)) +
  xlab('') + ylab('overlapping genes') -> plt_3D.plt
