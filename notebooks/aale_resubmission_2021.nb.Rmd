...
te_tf_plots <- function(ame_tf_list, upreg_te_list, de_list) {
  ...
  general_ame_tf_to_plt %>% 
    ...
    scale_color_manual(values = c(viridis(4)[1], viridis(4)[2], viridis(4)[3], 'grey'), name = 'sig. diff. exp. TF classification') +
...
te_znf_plots <- function(ame_znf_list, upreg_te_list, de_list) {
  ...
  tmp %>% 
    ...
    scale_color_manual(values = te_alone_color_pal, name = 'Clade of diff. exp. TE targeted by diff. exp. ZNF') +
    ylab('avg(ZNF expression)') +
    xlab('TE [with binding site] expression') +
...
znf_score_plt <- function(gencode_de_list, te_de_list) {
  ...
  tmp %>% 
    ...
    ggrepel::geom_text_repel(aes(label = ifelse(rank < 12, gene, '')), size = txt.mm_to_pts(0.8)) +
    xlab('') +
    theme(legend.position = c(0.99,0.99),
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_point() + 
  scale_color_viridis_d(name = 'ZNF type Imbeault, et al') +
  xlab('AALE log2FoldChange') + ylab('HBEC log2FoldChange') +
  geom_hline(yintercept = 0, linetype = 'dashed', alpha = 0.4) + 
  geom_vline(xintercept = 0, linetype = 'dashed', alpha = 0.4) +
  scale_x_continuous(limits = c(-3,3)) +
  scale_y_continuous(limits = c(-2,2)) +
  geom_text_repel(aes(label = ifelse(AALE < -1 & HBEC < -0.35, gene, '')), show.legend = F, size = txt.mm_to_pts(0.8)) + 
...
p4 <- ggplot(data.frame(l = intra.aale.te_ranges_list$plts$volcano_plt$labels$x, x = 1, y = 1)) +
...
wrap_elements(full = wrap_plots(guide_area(),
                              intra.aale.te_ranges_list$plts$volcano_plt + theme(legend.direction = 'horizontal') + ggtitle('AALE intra.') + xlab(''),
                              exo.aale.te_ranges_list$plts$volcano_plt + theme(legend.direction = 'horizontal') + ggtitle('AALE exo.') + ylab('') + xlab(''),
                              hbec.aale.te_ranges_list$plts$volcano_plt + theme(legend.direction = 'horizontal') + ggtitle('HBEC intra.') + ylab('') + xlab(''), 
                              p4,
...
p4 <- ggplot(data.frame(l = intra.aale.te_ranges_list$znf_plts$scatter_plt$labels$x, x = 1, y = 1)) +
      geom_text(aes(x, y, label = l), angle = 0, size = txt.mm_to_pts(0.8)) + 
      theme_void() +
      coord_cartesian(clip = "off")
  
wrap_elements(full = wrap_plots(guide_area(),
                              intra.aale.te_ranges_list$znf_plts$scatter_plt + theme(legend.direction = 'horizontal') + ggtitle('AALE intra.') + xlab(''),
                              exo.aale.te_ranges_list$znf_plts$scatter_plt + theme(legend.direction = 'horizontal') + ggtitle('AALE exo.') + ylab('') + xlab(''),
                              hbec.aale.te_ranges_list$znf_plts$scatter_plt + theme(legend.direction = 'horizontal') + ggtitle('HBEC intra.') + ylab('') + xlab(''), 
                              p4,
...
intra.aale.te_ranges_list$znf_plts$de_list %>% 
  ...
  facet_col(~clade) +
  geom_vline(xintercept = 0, linetype = 'dashed', alpha = 0.4) +
  ggtitle('AALE intra. DE ZNFs targeting TE clades') -> plt_S4A.plt
...
exo.aale.te_ranges_list$znf_plts$de_list %>% 
  ...
  facet_col(~clade) +
  geom_vline(xintercept = 0, linetype = 'dashed', alpha = 0.4) +
  ggtitle('AALE exo. DE ZNFs targeting TE clades') -> plt_S4B.plt
...
hbec.aale.te_ranges_list$znf_plts$de_list %>% 
  ...
  facet_col(~clade) +
  geom_vline(xintercept = 0, linetype = 'dashed', alpha = 0.4) +
  ggtitle('HBEC intra. DE ZNFs targeting TE clades') -> plt_S4C.plt
...
a549_de$de_682 %>% 
  ...
  ylab('AALE log2FC') + xlab('A549 log2FC') -> plt_5C.plt
...
wrap_plots(intra.aale.te_ranges_list$znf_plts$target_plt + theme(legend.position = c(0.3,0.3)) + ggtitle('Hughes, et al motif binding'),
           intra.aale.te_ranges_list$znf_plts$znf_score + theme(legend.position = c(0.99,0.99)) + ggtitle('Imbeault, et al binding score rank'),
           plt_5C.plt,
           plt_5D.plt,
           design = fig_5_patch) + 
...
wrap_plots(exo.aale.te_ranges_list$znf_plts$znf_score + theme(legend.position = c(0.99,0.99)) + ggtitle('AALE exo-Imbeault, et al binding score rank'),
           hbec.aale.te_ranges_list$znf_plts$znf_score + theme(legend.position = c(0.99,0.99)) + ggtitle('HBEC-Imbeault, et al binding score rank'),
           design = fig_S5_patch) + 
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_text_repel(aes(label = ifelse(`unique peaks` == 'ZNF' | abs(log2FoldChange) > 4, gene, '')), 
                  size = txt.mm_to_pts(0.8), show.legend = F, max.overlaps = 30) + 
  geom_vline(xintercept = c(0, -4.5), alpha = c(0.3,0.2), linetype = 'dashed') + 
  theme(legend.position = c(0.30, 0.99)) -> plt_6B.plt
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  geom_point() + 
  scale_color_viridis_d(name = 'sig. DE TF classification') +
...
# working_toil_counts %>% 
#   filter(gene %in% c(aale_znf$gene, aale_ifn$gene)) %>%
#   group_by(gene) %>%
#   mutate(count = scale(log1p(count), center = T)) %>%
#   select(sample, gene, count) %>% 
#   pivot_wider(names_from = gene, values_from = count) %>% 
...
#   select(gene) %>%
#   distinct() %>%
#   mutate(gene_set = ifelse(gene %in% aale_znf$gene, 'ZNF', 
#                            ifelse(gene %in% aale_ifn$gene, 'IFN', 'other'))) %>% 
#   column_to_rownames('gene') %>% 
#   ComplexHeatmap::rowAnnotation(df =.,
#                                 col = list(gene_set = c('ZNF' = viridis(2)[1], 'IFN' = viridis(3)[2])),
#                                 show_legend = F,
#                                 annotation_name_gp = grid::gpar(fontsize = 8),
#                                 simple_anno_size = unit(3, "mm")) -> gene_motif_anno
# 
# working_toil_counts %>%
#   filter(gene %in% c(aale_znf$gene, aale_ifn$gene)) %>%
#   select(gene) %>%
...
#                                 show_legend = T,
#                                 annotation_name_gp = grid::gpar(fontsize = 8),
#                                 simple_anno_size = unit(3, "mm")) -> gene_motif_anno_col
# 
# 
# 
# ComplexHeatmap::Heatmap(cor(t(toil_hm_counts)),
#                         name = 'pearson',
#                         show_row_names = F, show_column_names = F,
#                         left_annotation = log2fc_ha,
#                         right_annotation = gene_motif_anno, bottom_annotation = gene_motif_anno_col)
...
working_toil_counts %>% 
  ...
  ylab('z-score') + xlab('') + 
  scale_color_manual(values = c(viridis(3)[2], viridis(3)[1])) + 
  theme(axis.text.x = element_text(angle = 40, hjust = 1),
