...
unique_peak_aale_ifn_list <- unique(c(uniq_kras_peak_genes$`Gene Name`, aale_ifn$gene))
...
ifn_high_hm_zoom <- aale_ifn_chm[aale_ifn_chm_rowOrder[1:18], ]

aale_ifn_chm_no_rownames <- plot_complex_heatmap(unique_peak_aale_ifn_list, show_genes = F, split_vector = 2, split_title = "cluster_%s")
aale_ifn_chm_no_rownames
```
## patchwork
```{r}
...
salmon.quant$aale$intra$deseq$de %>% 
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(abs(log2FoldChange) > 5 |
                                                -log10(padj) > 250, gene, '')),
                           size = txt.mm_to_pts(0.8)) +
  facet_col(~biotype, scales = 'free') -> plt_S1A.plt
...
salmon.quant$aale$intra$deseq$pca$pca.out %>% 
  ...
  ggplot(aes(PC1, PC2,
    ...
    xlab(paste('PC1', round(cumsum(salmon.quant$aale$intra$deseq$pca$pcs.props)[1], digits = 3), sep = ' ')) +
    ylab(paste('PC2', round(cumsum(salmon.quant$aale$intra$deseq$pca$pcs.props)[2] - cumsum(salmon.quant$aale$intra$deseq$pca$pcs.props)[1],
                            digits = 3), sep = ' ')) +
    scale_color_manual(values = c(viridis(3)[1], viridis(3)[2])) +
